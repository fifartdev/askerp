...
export default function EditOrderPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/service-orders/${id}?depth=1`, { credentials: 'include' }).then((r) => r.json()),
      ...
      setFields({
        client: typeof order.client === 'object' ? order.client?.id : (order.client ?? ''),
        category: typeof order.category === 'object' ? order.category?.id : (order.category ?? ''),
  ...
  return (
    <div className="max-w-xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href={`/dashboard/orders/${id}`} className="text-gray-400 hover:text-gray-600 transition-colors">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
        <h2 className="text-xl font-semibold text-gray-900">Επεξεργασία Εντολής</h2>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <Link href={`/dashboard/orders/${id}`} className="text-sm text-gray-500 hover:text-gray-700 transition-colors">
              Ακύρωση
            </Link>
          </div>
        </form>
